...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (target.id === message.author.id) return message.react(client.emojis.cache.find(x => x.name === "ramal_iptal"))
    ...
    target.roles.set(unRegisterRol).then(x => message.react(client.emojis.cache.find(x => x.name === "ramal_tik"))).catch(y => message.react(client.emojis.cache.find(x => x.name === "ramal_iptal")));
	} else return;
}
