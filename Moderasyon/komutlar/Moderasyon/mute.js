...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (sec == "mutesetup") {
        ...
        if (message.guild.members.cache.some(member => conf.sahip.some(sahip => member === sahip)) || message.member.permissions.has(8) || message.author.id === message.guild.owner.id) {
            await sunucuayar.findOne({
                ...
                if (args[1] == "yetki") {
                    ...
                    return data.MUTEAuthorized = select, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "ramal_tik")))
                }
                if (args[1] == "kanal") {
                    let select = message.mentions.channels.first();
                    if (!select) return message.react(client.emojis.cache.find(res => res.name === "ramal_iptal"));
                    return data.MUTEChannel = select.id, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "ramal_tik")))
                }
                if (args[1] == "limit") {
                    let select = Number(args[2])
                    if (!select) return message.react(client.emojis.cache.find(res => res.name === "ramal_iptal"));
                    return data.MUTELimit = select, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "ramal_tik")))
                }
            })
        ...
        if (message.guild.members.cache.some(member => conf.sahip.some(sahip => member === sahip)) || message.member.permissions.has(8) || message.author.id === message.guild.owner.id) {
            await sunucuayar.findOne({
                ...
                if (args[1] == "yetki") {
                    ...
                    return data.VMUTEAuthorized = select, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "ramal_tik")))
                }
                if (args[1] == "kanal") {
                    let select = message.mentions.channels.first();
                    if (!select) return message.react(client.emojis.cache.find(res => res.name === "ramal_iptal"));
                    return data.VMUTEChannel = select.id, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "ramal_tik")))
                }
                if (args[1] == "limit") {
                    let select = Number(args[2])
                    if (!select) return message.react(client.emojis.cache.find(res => res.name === "ramal_iptal"));
                    return data.VMuteLimit = select, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "ramal_tik")))
                }
            })
        } else return message.reply("Bu komutu kullanabilmek için YÖNETİCİ - Sunucu Sahibi olmanız gerekiyor")
    } else {
        ...
        await message.react(`${client.emojis.cache.find(x => x.name === "ramal_mute")}`);
        await message.react(`${client.emojis.cache.find(x => x.name === "ramal_vmute")}`);

        const filter = (reaction, user) => {
            return ["ramal_mute", "ramal_vmute"].includes(reaction.emoji.name) && user.id === message.author.id;
        };
        ...
        collector.on("collect", async (reaction) => {
            ...
            message.react(`${client.emojis.cache.find(x => x.name === "ramal_tik")}`);
            if (reaction.emoji.name == "ramal_mute") {
                if (await client.permAyar(message.author.id, message.guild.id, "mute") || durum) {
                    ...
                    if (muteSorumlusu.length >= 1 && client.channels.cache.get(muteLogKanal) && muteLimit >= 1) {
                        if (target.roles.cache.get(muteROL)) return message.react(client.emojis.cache.find(res => res.name === "ramal_iptal"));
...
`)
            ...
            } else if (reaction.emoji.name == "ramal_vmute") {
