...
module.exports.run = async (client, message, args, durum, kanal) => {
        ...
        if (message.guild.members.cache.some(member => conf.sahip.some(sahip => member === sahip)) || message.member.permissions.has(8) || message.author.id === message.guild.owner.id) {
            await sunucuayar.findOne({
                ...
                if (args[1] == "yetki") {
                    ...
                    return data.REKLAMAuthorized = select, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "ramal_tik")))
                }
                if (args[1] == "kanal") {
                    let select = message.mentions.channels.first();
                    if (!select) return message.react(client.emojis.cache.find(res => res.name === "ramal_iptal"));
                    return data.REKLAMChannel = select.id, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "ramal_tik")))
                }
                if (args[1] == "limit") {
                    let select = Number(args[2])
                    if (!select) return message.react(client.emojis.cache.find(res => res.name === "ramal_iptal"));
                    return data.REKLAMLimit = select, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "ramal_tik")))
                }
            })
    ...
    if (await client.permAyar(message.author.id, message.guild.id, "reklam")) {
        if (jailSorumlusu.length >= 1 && client.channels.cache.get(reklamLOGKanal) && REKLAMLimit >= 1) {
            ...
            if (target.roles.cache.get(reklamROL)) return message.react(client.emojis.cache.find(res => res.name === "ramal_iptal"));
