...
module.exports.run = async (client, message, args, durum, kanal) => {
        ...
        if (message.guild.members.cache.some(member => conf.sahip.some(sahip => member === sahip)) || message.member.permissions.has(8) || message.author.id === message.guild.owner.id) {
            await sunucuayar.findOne({
                ...
                if (args[1] == "yetki") {
                    ...
                    return data.JAILAuthorized = select, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "ramal_tik")))
                }
                if (args[1] == "kanal") {
                    let select = message.mentions.channels.first();
                    if (!select) return message.react(client.emojis.cache.find(res => res.name === "ramal_iptal"));
                    return data.JAILChannel = select.id, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "ramal_tik")))
                }
                if (args[1] == "limit") {
                    let select = Number(args[2])
                    if (!select) return message.react(client.emojis.cache.find(res => res.name === "ramal_iptal"));
                    return data.JAILLimit = select, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "ramal_tik")))
                }
            })
    ...
    if (await client.permAyar(message.author.id, message.guild.id, "jail") || durum) {
        if (JailSorumlusu.length >= 1 && client.channels.cache.get(JailLogKanal) && JailLimit >= 1) {
            ...
            if (target.roles.cache.get(JailROL)) return message.react(client.emojis.cache.find(res => res.name === "ramal_iptal"));
