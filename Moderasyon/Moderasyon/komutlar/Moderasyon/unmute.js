...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    await message.react(`${client.emojis.cache.find(x => x.name === "ramal_unmute")}`);
    await message.react(`${client.emojis.cache.find(x => x.name === "ramal_vunmute")}`);
    const filter = (reaction, user) => {return ["ramal_unmute", "ramal_vunmute"].includes(reaction.emoji.name) && user.id === message.author.id;};
    ...
    collector.on("collect", async (reaction) => {
        ...
        if(reaction.emoji.name == "ramal_unmute") {
        ...
        } else if (reaction.emoji.name == "ramal_vunmute") {
